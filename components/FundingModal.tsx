...
export function FundingModal({
  ...
  const onSubmit = async (data: FundingFormData) => {
    ...
    try {
      ...
      if (fundingType === "bank" && !data.routingNumber) {
        setError("Routing number is required for bank funding");
        return;
      }

      if (fundingType === "bank") {
        // use transferFromAccount for bank funding
        ...
        await fundAccountUsingCardMutation.mutateAsync({
          accountId,
          amount,
          fundingSource: {
      ...
      // Invalidate transactions cache to reflect new funding
      trpcUtils.account.getTransactions.invalidate({
        accountId: accountId,
      });
      onSuccess();
    } catch (err: any) {
